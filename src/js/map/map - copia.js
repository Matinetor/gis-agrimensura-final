...
fetch('data/facultades.json')
    .then(res => res.json())
    .then(data => {
        window.datosFacultadesGlobal = data.features;
        activarBuscadorFacultades(); // Iniciar buscador una vez que hay datos
        
        // Agregar los puntos al mapa
        L.geoJson(data, {
            onEachFeature: function(feature, layer) {
                layer.on('click', () => actualizarSidebarFacultad(feature.properties));
            }
        }).addTo(mapa);
    });
        ...
        case "Draw":
          ...
          mapa.activeLayerHasChanged = (layer, addToList) => {
            ...
            if (containerIdx >= 0 && !addToList) {
              activeLayersDiv.removeChild(
                activeLayersDivChilds[containerIdx]
              );
  ...
  function parseFeatureInfoHTML(info, idTxt) {
    ...
    if (infoAux > 0) {
      ...
      if (infoAux > 0) {
        ...
        return (
          '<div class="featureInfo" id="featureInfoPopup' + idTxt + '"><div class="featureGroup"><div style="padding:1em;overflow:scroll-x;overflow-y:hidden" class="individualFeature">' +
          info +
          "</div></div></div>"
        );
      }
...
/* ==========================================================
   LÓGICA PERSONALIZADA: SIDEBAR DE OBRAS (GeoServer JSON)
   ========================================================== */

// Configuración: nombres de capas WMS que activan la sidebar
// Puede ser un string (nombre exacto) o un array de nombres
// Si es null o vacío, se aplica a todas las capas WMS
var SIDEBAR_OBRAS_LAYER_FILTER = null; // null = todas las capas WMS, o ['obras', 'parcelas'] para específicas

// Esperamos a que el mapa y las dependencias estén listos
var checkMapInterval = null;
var checkMapAttempts = 0;
var MAX_CHECK_ATTEMPTS = 60; // 30 segundos máximo (500ms * 60)
var sidebarScriptLoaded = false;

// Función para cargar dinámicamente leaflet-sidebar-v2
function loadSidebarScript(callback) {
    if (sidebarScriptLoaded || (typeof L !== 'undefined' && typeof L.control !== 'undefined' && typeof L.control.sidebar === 'function')) {
        sidebarScriptLoaded = true;
        if (callback) callback();
        return;
    }
    
    if (document.querySelector('script[src*="leaflet-sidebar"]')) {
        // El script ya está en el DOM, esperar a que cargue
        var checkScript = setInterval(function() {
            if (typeof L !== 'undefined' && typeof L.control !== 'undefined' && typeof L.control.sidebar === 'function') {
    ...
    script.onerror = function() {
        console.error("Error al cargar leaflet-sidebar-v2");
    }
    document.head.appendChild(script);
}

checkMapInterval = setInterval(function() {
  checkMapAttempts++;
  
  if (typeof mapa !== 'undefined' && mapa !== "" && typeof L !== 'undefined') {
      // Leaflet está disponible, cargar sidebar script si no está cargado
      loadSidebarScript(function() {
          if (typeof L.control !== 'undefined' && typeof L.control.sidebar === 'function') {
              clearInterval(checkMapInterval);
              checkMapInterval = null;
              initSidebarObras();
          }
      });
  } else if (checkMapAttempts >= MAX_CHECK_ATTEMPTS) {
      clearInterval(checkMapInterval);
      checkMapInterval = null;
      console.warn("⚠️ No se pudo inicializar Sidebar de Obras: timeout alcanzado");
  }
}, 500);

var sidebarObrasInitialized = false;

function initSidebarObras() {
  // Evitar inicialización múltiple
  if (sidebarObrasInitialized || window.sidebarRight) {
      return;
  }

  try {
      console.log("Iniciando Sidebar de Obras...");

      // 1. Crear la Sidebar GLOBALMENTE
      window.sidebarRight = L.control.sidebar({
      ...
      sidebarObrasInitialized = true;

      // 2. Interceptar click en el mapa para capas WMS específicas
      mapa.on('click', function(e) {
          var layerFound = false;
          
          mapa.eachLayer(function(layer) {
              // Verificar si es una capa WMS
              var isWMSLayer = (layer instanceof L.TileLayer.WMS) || 
                              (layer._url && layer.options && layer.options.layers);
              
              if (!isWMSLayer) {
                  return;
              }

              // Aplicar filtro de capas si está configurado
              var layerName = layer.options ? layer.options.layers : '';
              if (SIDEBAR_OBRAS_LAYER_FILTER !== null && SIDEBAR_OBRAS_LAYER_FILTER !== '') {
                  var filterArray = Array.isArray(SIDEBAR_OBRAS_LAYER_FILTER) ? 
                                    SIDEBAR_OBRAS_LAYER_FILTER : 
                                    [SIDEBAR_OBRAS_LAYER_FILTER];
                  
                  var layerMatches = false;
                  if (typeof layerName === 'string') {
                      layerMatches = filterArray.some(function(filter) {
                          return layerName.indexOf(filter) !== -1;
                      });
                  } else if (Array.isArray(layerName)) {
                      layerMatches = layerName.some(function(name) {
              ...
              // Si llegamos aquí, procesar la capa
              if (!layerFound) {
                  layerFound = true;
                  
                  // Cerrar popups de Argenmap antes de mostrar la sidebar
                  mapa.closePopup();
                  ...
                  var url = getFeatureInfoUrl(
                      ...
                      e.latlng,
                      {
                          ...
                          'propertyName': 'id_cli,nombre,contacto,tel,mail,legajo_lista'
                      }
                  );
      ...
      console.log("✅ Sidebar de Obras inicializada correctamente.");
  } catch (error) {
      console.error("Error al inicializar Sidebar de Obras:", error);
  }
}

// Función auxiliar URL WMS - Compatible con WMS 1.1.1 y 1.3.0
function getFeatureInfoUrl(map, layer, latlng, params) {
  try {
      ...
      // Obtener versión WMS (por defecto 1.1.1)
      var wmsVersion = (layer.options && layer.options.version) || '1.1.1';
      var isWMS130 = (wmsVersion === '1.3.0');
      
      // Construir bbox según versión WMS
      var bboxValue;
      if (isWMS130) {
          // WMS 1.3.0: minx,miny,maxx,maxy (orden diferente)
          bboxValue = sw.lng + ',' + sw.lat + ',' + ne.lng + ',' + ne.lat;
      } else {
          // WMS 1.1.1: minx,miny,maxx,maxy
          bboxValue = sw.lng + ',' + sw.lat + ',' + ne.lng + ',' + ne.lat;
      }
      ...
      var defaultParams = {
          ...
          service: 'WMS',
          srs: isWMS130 ? 'EPSG:4326' : 'EPSG:4326',
          styles: (layer.options && layer.options.styles) || '',
          version: wmsVersion,
          transparent: (layer.options && layer.options.transparent) !== undefined ? layer.options.transparent : true,
          format: (layer.options && layer.options.format) || 'image/png',
          bbox: bboxValue,
          height: size.y,
          width: size.x,
      ...
      // Extender con parámetros personalizados (como propertyName)
      params = L.Util.extend(defaultParams, params || {});
      
      // Coordenadas según versión WMS
      if (isWMS130) {
      ...
      console.error("Error al construir URL GetFeatureInfo:", error);
      return null;
  }
}

// 3. Renderizar HTML - Función robusta con validación completa
function mostrarFichaEnSidebar(datos) {
  try {
      // Validación y normalización de datos
      var idCli = datos.id_cli || '0000';
      // Asegurar formato de 4 dígitos
      idCli = String(idCli).padStart(4, '0');
      
      var nombre = datos.nombre || 'Sin Nombre';
      var contacto = datos.contacto || 'S/D';
      var tel = datos.tel || '';
      var mail = datos.mail || '';
      
      // Procesar legajo_lista
      var legajoLista = datos.legajo_lista || '';
      var listaCodigos = [];
      
      if (legajoLista && typeof legajoLista === 'string' && legajoLista.trim() !== '') {
          listaCodigos = legajoLista.split(',').map(function(cod) {
              return cod.trim();
          }).filter(function(cod) {
              return cod !== '';
          });
      ...
      // Separar fotos (F_) de documentos (A_, B_, C_, D_)
      var fotos = listaCodigos.filter(function(c) {
          return c.toUpperCase().startsWith('F_');
      });
      
      var documentos = listaCodigos.filter(function(c) {
          var upperC = c.toUpperCase();
          return upperC.startsWith('A_') || 
                 upperC.startsWith('B_') || 
                 upperC.startsWith('C_') || 
                 upperC.startsWith('D_');
      });
      
      // Función auxiliar para mapear sigla a carpeta y extensión
      function obtenerRutaArchivo(codigo) {
          var cod = codigo.trim().toUpperCase();
          var sigla = cod.substring(0, 2); // A_, B_, C_, D_, F_
          
          var mapeo = {
              'A_': { carpeta: '01_LEGAL', extension: '.jpeg' },
              'B_': { carpeta: '02_AGRIMENSURA', extension: '.pdf' },
              'C_': { carpeta: '03_TOPOGRAFIA', extension: '.pdf' },
              'D_': { carpeta: '04_OBRA', extension: '.pdf' },
              'F_': { carpeta: '04_OBRA', extension: '.jpeg' }
          };
          
          var config = mapeo[sigla] || { carpeta: '04_OBRA', extension: '.pdf' };
          return {
              carpeta: config.carpeta,
              extension: config.extension,
              nombreCompleto: cod + config.extension,
              ruta: 'assets/proyectos/ID_' + idCli + '/' + config.carpeta + '/' + cod + config.extension
          };
      }
      
      // Generar HTML de galería de fotos
      var htmlGaleria = '';
      if (fotos.length > 0) {
          htmlGaleria = '<div class="galeria-fotos" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; margin-bottom: 20px; max-height: 250px; overflow-y: auto; padding-right: 5px;">';
          fotos.forEach(function(foto) {
              var rutaFoto = obtenerRutaArchivo(foto);
              htmlGaleria += '<img src="' + rutaFoto.ruta + '" ' +
                           'style="width:100%; height:70px; object-fit:cover; border-radius:4px; cursor:pointer; border: 1px solid #ddd;" ' +
                           'onclick="window.open(this.src)" ' +
                           'onerror="this.src=\'src/styles/images/noimage.webp\'; this.onerror=null;" ' +
                           'title="' + foto + '">';
          });
          htmlGaleria += '</div>';
      } else {
          htmlGaleria = '<p style="color:#999; font-size:12px; padding: 10px; text-align: center;">No hay fotos cargadas.</p>';
      }
      
      // Generar HTML de lista de documentos
      var htmlDocumentos = '';
      if (documentos.length > 0) {
          htmlDocumentos = '<div class="list-group" style="max-height: 200px; overflow-y: auto;">';
          documentos.forEach(function(doc) {
              var rutaDoc = obtenerRutaArchivo(doc);
              htmlDocumentos += '<a href="' + rutaDoc.ruta + '" ' +
                               'target="_blank" ' +
                               'class="list-group-item" ' +
                               'style="font-size: 12px; padding: 8px;">' +
                               '<span class="glyphicon glyphicon-file"></span> ' + doc +
                               '</a>';
          });
          htmlDocumentos += '</div>';
      } else {
          htmlDocumentos = '<p style="color:#999; font-size:12px; padding: 10px; text-align: center;">Sin documentos registrados.</p>';
      }
      
      // Generar botones de contacto
      var htmlContacto = '';
      if (tel || mail) {
          htmlContacto = '<div class="panel-contacto" style="background: #f8f9fa; border: 1px solid #e0e0e0; padding: 10px; border-radius: 8px; margin-bottom: 20px;">';
          htmlContacto += '<p style="margin: 0; font-size: 13px;"><strong>👤 Responsable:</strong> ' + contacto + '</p>';
          
          if (tel || mail) {
              htmlContacto += '<div style="display: flex; gap: 5px; margin-top: 10px;">';
              if (tel) {
                  // Limpiar el número de teléfono para WhatsApp (eliminar espacios, guiones, paréntesis, etc.)
                  var whatsappNumber = tel.replace(/[\s\-\(\)]/g, '');
                  // Asegurar que tenga el código de país (si no lo tiene, asumir +54 para Argentina)
                  if (!whatsappNumber.startsWith('+')) {
                      if (whatsappNumber.startsWith('54')) {
                          whatsappNumber = '+' + whatsappNumber;
                      } else if (whatsappNumber.startsWith('9')) {
                          // Si empieza con 9, asumir que es un número argentino sin código de país
                          whatsappNumber = '+549' + whatsappNumber.substring(1);
                      } else {
                          whatsappNumber = '+54' + whatsappNumber;
                      }
                  }
                  var whatsappUrl = 'https://wa.me/' + whatsappNumber.replace(/\+/g, '');
                  htmlContacto += '<a href="' + whatsappUrl + '" target="_blank" class="btn btn-success btn-xs" style="flex: 1; background-color: #25D366; border-color: #25D366;">💬 WhatsApp</a>';
              }
              if (mail) {
                  htmlContacto += '<a href="mailto:' + mail + '" class="btn btn-info btn-xs" style="flex: 1;">✉️ Email</a>';
              }
              htmlContacto += '</div>';
          }
          htmlContacto += '</div>';
      ...
      var html = '<div class="ficha-obra">' +
                 '<div class="btn-cerrar-ficha" onclick="window.sidebarRight.close()" title="Cerrar ficha">×</div>' +
                 '<h3 class="ficha-titulo">' + nombre + '</h3>' +
                 '<p style="color: #666; font-weight: bold; margin-bottom:15px;">ID CLIENTE: ' + idCli + '</p>' +
                 htmlContacto +
                 '<h4 style="border-bottom: 2px solid #f39c12; padding-bottom: 5px; font-size: 15px; margin-top: 20px;">📸 Registro de Obra</h4>' +
                 htmlGaleria +
                 '<h4 style="border-bottom: 2px solid #3498db; padding-bottom: 5px; font-size: 15px; margin-top: 20px;">📄 Legajo y Planos</h4>' +
                 htmlDocumentos +
                 '</div>';
      
      // Actualizar contenido y abrir sidebar
      var contenidoFicha = document.getElementById('contenido-ficha');
      if (contenidoFicha) {
          ...
          console.error("No se encontró el elemento 'contenido-ficha'");
      }
  } catch (error) {
      console.error("Error al mostrar ficha en sidebar:", error);
      var errorMsg = '<div class="ficha-obra" style="padding: 20px;">' +
                     '<p style="color: #d9534f;">Error al cargar la información. Por favor, intente nuevamente.</p>' +
                     '</div>';
      var contenidoFicha = document.getElementById('contenido-ficha');
      if (contenidoFicha) {
          contenidoFicha.innerHTML = errorMsg;
      }
  }
}

// Función global para compatibilidad y testing desde consola
window.mostrarFicha = function(datos) {
    // Si la sidebar ya está inicializada, usar directamente
    if (window.sidebarRight) {
        mostrarFichaEnSidebar(datos);
        return;
    }
    
    // Si no está inicializada, intentar inicializarla si las dependencias están listas
    if (typeof mapa !== 'undefined' && mapa !== "" && typeof L !== 'undefined') {
        // Cargar el script de sidebar si no está cargado
        loadSidebarScript(function() {
            if (typeof L.control !== 'undefined' && typeof L.control.sidebar === 'function') {
                console.log("Inicializando sidebar de obras...");
                initSidebarObras();
                ...
                setTimeout(function() {
                    if (window.sidebarRight) {
                        ...
                        console.error("No se pudo inicializar la sidebar. Verifica que el mapa esté completamente cargado.");
                    }
                }, 200);
            } else {
                console.error("No se pudo cargar leaflet-sidebar-v2. Intenta recargar la página.");
            }
        });
    } else {
        console.error("La sidebar de obras no está inicializada. Dependencias faltantes:");
        if (typeof mapa === 'undefined' || mapa === "") {
            console.error("  - El objeto 'mapa' no está disponible");
        }
        if (typeof L === 'undefined') {
            console.error("  - La librería Leaflet (L) no está cargada");
        }
        console.log("Espera a que el mapa esté completamente cargado y vuelve a intentar.");
    }
};
...
function activarBuscadorFacultades() {
  console.log("🎓 Inicializando Buscador de Facultades...");

  $("#q-obras").autocomplete({ // Mantenemos el ID para no cambiar el HTML
      ...
      select: function(event, ui) {
          ...
          actualizarSidebarFacultad(f.properties);
      }
  });
}

function actualizarSidebarFacultad(p) {
  const sidebar = $("#sidebar-contenido"); // Ajustar al ID de tu sidebar
  
  // Generamos rutas de logos basadas en el código (FIUBA.png, UNLP.png, etc.)
  const logoFacultad = `img/logos/${p.nombre_cod}.png`;
  const logoComision = p.com_cod ? `img/logos/${p.com_cod}.png` : 'img/logos/default_cea.png';

  let html = `
      <div class="detalle-facultad">
          <h3>${p.nombre} (${p.nombre_cod})</h3>
          <div class="logos-container" style="display: flex; gap: 10px; margin-bottom: 15px;">
              <img src="${logoFacultad}" onerror="this.src='img/default_univ.png'" style="width: 80px; height: auto;">
              ${p.com_cod ? `<img src="${logoComision}" style="width: 80px; height: auto;">` : ''}
          </div>
          
          <table class="table table-sm">
              <tr><th>ID:</th><td>${p.id}</td></tr>
              <tr><th>Comisión:</th><td>${p.comision || 'No registrada'}</td></tr>
          </table>

          <div class="links-utiles">
              <a href="${p.url_fac}" target="_blank" class="btn btn-primary btn-sm">Sitio Facultad</a>
              ${p.url_com ? `<a href="${p.url_com}" target="_blank" class="btn btn-info btn-sm">Instagram Comisión</a>` : ''}
          </div>
      </div>
  `;
  
  sidebar.html(html);
  if (typeof abrirSidebar === "function") abrirSidebar(); // Abrir si está cerrada
}